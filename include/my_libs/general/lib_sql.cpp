...
lib_sql::mySQLite::mySQLite(const char* db_file_nm, const bool read_only, int wait_max_seconds)
{
...
void lib_sql::mySQLite::open(const char* db_file_nm, const bool read_only)
{
    // SQLite status code
    int sts;
...
void lib_sql::mySQLite::close() const
{
    // SQLite status code
    int sts;
    int counter = this->wait_max_seconds;
...
void lib_sql::mySQLite::vacuum() const
{
    // SQLite status code
    int sts;
    int counter = this->wait_max_seconds;
...
void lib_sql::mySQLite::exec(const std::string &sql) const
{
    // SQLite status code
    int sts;
    int counter = this->wait_max_seconds;
...
// check column type
bool check_type(sqlite3_stmt* stmt, std::shared_ptr<lib_df::myDataFrame> rslt, int cols_no)
{
...
void add_row(sqlite3_stmt* stmt, std::shared_ptr<lib_df::myDataFrame> rslt, int cols_no)
{
...
std::shared_ptr<lib_df::myDataFrame> lib_sql::mySQLite::query(const std::string &sql) const
{
    // SQLite status code
    int sts;
...
std::shared_ptr<lib_df::myDataFrame> lib_sql::mySQLite::download_tbl(const std::string &tbl_nm)
{
    // dataframe
    std::shared_ptr<lib_df::myDataFrame> rslt = std::make_shared<lib_df::myDataFrame>();
...
void lib_sql::mySQLite::upload_tbl(const lib_df::myDataFrame &tbl, const std::string &tbl_nm, const bool delete_old_data)
{
    // variables
    int cols_no;
    long rows_no;
    ...
    // create table if it does not exist
        ...
        for (col_idx = 0; col_idx < cols_no; col_idx++)
        {
            ...
            dtype = lib_str::to_upper(tbl.tbl.dtypes[col_idx]);
...
std::string lib_sql::read_sql(const std::string& sql_file_nm, const std::string& tag)
{
    // declare file stream
    std::ifstream f;
    ...
    std::string tag_aux = "###!";
    std::string tag_updated = tag_aux + tag;
    ...
    if (f.is_open())
    {
        while(getline(f, line))
        {
            ...
            if (sql_query_found && (line.substr(0, 4).compare(tag_aux) == 0))
            {
                ...
                if (sql.compare("") == 0)
                {
                    throw std::runtime_error((std::string)__func__ + ": Tag " + tag_updated + " not found in file " + sql_file_nm + "!");
                }
            ...
            if (line.substr(0, tag_updated.length()).compare(tag_updated) == 0)
...
std::string lib_sql::replace_in_sql(std::string sql, std::string replace_what, std::string replace_with)
{
